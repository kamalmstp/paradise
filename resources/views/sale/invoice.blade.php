...
<div style="max-width:400px;margin:0 auto">
    ...
    <div id="receipt-data">
        <div class="centered">
            ...
            <h2 style="margin-top: -10px;">{{$lims_biller_data->company_name}}</h2>
        ...
        <p>
            ...
            {{trans('file.Date')}} : {{date('d-m-Y', strtotime($lims_sale_data->created_at))}}<br>
        ...
        <table>
            <tbody>
                ...
                <tr>
                    <td colspan="2">
                        {{$product_name}}
                        <br>{{$product_sale_data->qty}} x {{config('currency')." ".number_format((float)($product_sale_data->total / $product_sale_data->qty))}}
                    ...
                    <td style="text-align:right;vertical-align:bottom">{{config('currency')." ".number_format((float)$product_sale_data->total)}}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr>
                    ...
                    <th style="text-align:right">{{config('currency')." ".number_format((float)$lims_sale_data->total_price)}}</th>
                </tr>
                @if($general_setting->invoice_format == 'gst' && $general_setting->state == 1)
                <tr>
                    <td colspan="2">IGST</td>
                    <td style="text-align:right">{{config('currency')." ".number_format((float)$total_product_tax)}}</td>
                </tr>
                @elseif($general_setting->invoice_format == 'gst' && $general_setting->state == 2)
                <tr>
                    <td colspan="2">SGST</td>
                    <td style="text-align:right">{{config('currency')." ".number_format((float)($total_product_tax / 2))}}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <th style="text-align:right">{{config('currency')." ".number_format((float)$lims_sale_data->order_tax)}}</th>
                </tr>
                @endif
                @if($lims_sale_data->order_discount)
                <tr>
                    <th colspan="2">{{trans('file.Order Discount')}}</th>
                    <th style="text-align:right">{{config('currency')." ".number_format((float)$lims_sale_data->order_discount)}}</th>
                </tr>
                @endif
                @if($lims_sale_data->coupon_discount)
                <tr>
                    <th colspan="2">{{trans('file.Coupon Discount')}}</th>
                    <th style="text-align:right">{{config('currency')." ".number_format((float)$lims_sale_data->coupon_discount)}}</th>
                </tr>
                @endif
                @if($lims_sale_data->shipping_cost)
                <tr>
                    <th colspan="2">{{trans('file.Shipping Cost')}}</th>
                    <th style="text-align:right">{{config('currency')." ".number_format((float)$lims_sale_data->shipping_cost)}}</th>
                </tr>
                @endif
                <!-- <tr>
                    <th colspan="2">{{trans('file.grand total')}}</th>
                    <th style="text-align:right">{{config('currency')." ".number_format((float)$lims_sale_data->grand_total)}}</th>
                </tr> -->
                <!-- <tr>
        ...
        <table>
            <tbody>
                ...
                <tr style="background-color:#ddd;">
                    ...
                    <td style="padding: 5px;width:30%">{{trans('file.Change')}}: {{config('currency')." ".number_format((float)$payment_data->change)}}</td>
                    <td style="padding: 5px;width:40%">{{trans('file.Amount')}}: {{config('currency')." ".number_format((float)$payment_data->amount)}}</td>
                    
                </tr>                
                @endforeach
                <tr><td class="centered" colspan="3">{{trans('file.Thank you for shopping with us. Please come again')}} <br><p>@paradise.bdj</p></td></tr>
                <!-- <tr>
